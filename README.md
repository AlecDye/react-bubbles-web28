...
A: Tokens are used for authenticating a user. When a user submits form data (usually a username and a password) that data is then encrypted when it is sent to the web server. The encryption (token) is unique to that user and saved in local storage on the user's browser.

- [ ] What steps can you take in your web apps to keep your data secure?

A: Using tokens along with private routes is the most common way of protecting data on a website. Using session data is more secure than using tokens in local storage.

- [ ] Describe how web servers work.

A: A web server is an internet-connected computer that receives a browser request (http request) to view a website. It "hosts" the website which is then viewed/accessed by the browser.
